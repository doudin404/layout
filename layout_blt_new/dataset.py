    ...
    def render(self, layout):
        ...
        for i in range(len(layout)):
            ...
            if i > 0:
                ...
                x_center_last, y_center_last = (x1_last+x2_last)/2, (y1_last+y2_last)/2
...
def reading_order(bboxs, indices=None):
    ...
    for col_cut in x2[sort_indices]:
        ...
        if len(right_half) == 0:
            break
        elif len(left_half) + len(right_half) == len(indices):
            return np.concatenate((reading_order(bboxs, np.array(left_half)), reading_order(bboxs, np.array(right_half))))

    # 从上至下遍历所有元素的底边
    sort_indices = indices[np.argsort(y2[indices])]
    top_half = []
    bottom_half, not_top_half = list(reversed(indices[np.argsort(y1[indices])])), list(
        reversed(indices[np.argsort(y2[indices])]))

    for rol_cut in y2[sort_indices]:
        ...
        if len(bottom_half) == 0:
            break
        if len(top_half) + len(bottom_half) == len(indices):
            return np.concatenate((reading_order(bboxs, np.array(top_half)), reading_order(bboxs, np.array(bottom_half))))
