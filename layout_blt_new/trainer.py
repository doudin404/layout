...
import logging
import wandb
from sample import sample1 as sample
...
class Trainer:
    ...
    def run_epoch_(self, epoch, split):
        ...
        for it, y in pbar:
            ...
            x, masks = BLT_masking(
                y, self.train_dataset.mask_token, self.train_dataset.eos_token, self.train_dataset.pad_token, mask_rate=mask_rate, training=is_train)
...
def attribute_random_masking(inputs, mask_token, pad_token):
    """
    ...
    mask_lens = torch.ceil(
        lens * mask_rate).clone().detach().requires_grad_(True)
